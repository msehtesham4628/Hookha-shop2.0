...
const aliases: Record<string, string[]> = {
  ...
  images: ['images', 'image urls', 'image urls'],
...
function normalizeImages(row: Record<string, unknown>, name: string) {
  ...
  if (rawImages) {
    urls.push(...String(rawImages).split(/[|,\n]+/).map(v => v.trim()).filter(Boolean));
  }
...
function rowToProduct(row: Record<string, unknown>, existing?: Product): Product {
  ...
  product.categorySlug = slugify(category);
...
// Accepts rows parsed from an Excel workbook. SKU is the stable product key.
// The workbook is authoritative: rows are upserted and products missing from
// the workbook are removed from MongoDB and the in-memory store.
router.post('/products/excel-sync', async (req: AuthenticatedRequest, res) => {
  ...
  for (let i = 0; i < normalizedRows.length; i++) {
    ...
    if (!sku || !name || price === undefined) {
      errors.push(`Row ${i + 2}: SKU, Name and Price are required.`);
      continue;
    }
    if (seenSkus.has(sku)) errors.push(`Row ${i + 2}: duplicate SKU '${sku}'.`);
    seenSkus.add(sku);
  }
  ...
  if (!connected) {
    return res.status(503).json({ success: false, error: { code: 'MONGODB_UNAVAILABLE', message: 'MongoDB is not connected. Excel sync was not applied.' } });
  }
  ...
  try {
    ...
    for (const row of normalizedRows) {
      ...
      const product = rowToProduct(row, bySku.get(sku));
    ...
    for (const product of removed) {
      await mongoService.deleteDocument('products', { id: product.id });
    }

    // Make the current Lambda memory exactly match the workbook immediately.
    db.products = imported;
    db.persistenceData.deletedProductIds = db.persistenceData.deletedProductIds.filter(id => imported.some(p => p.id === id));
